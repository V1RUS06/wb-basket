...
function addListenersOnArrows() {
  ...
  const activeBasketContainer = document.getElementById(
    "active_basket_container"
  );
  const disabledBasketContainer = document.getElementById(
    "disabled_basket_container"
  );

  firstArrow.addEventListener("click", (e) =>
    rotateArrowAndHideContainer(activeBasketContainer, e.target)
  );
  secondArrow.addEventListener("click", (e) =>
    rotateArrowAndHideContainer(disabledBasketContainer, e.target)
  );
}

function rerenderBasketCount() {
  const selectedItemCount = Cart.getSelectedItems();
  const basketCircle = document.querySelector(".header_basket_icon_span");
  const basketCircleMobile = document.getElementById("nav_basket-circle");

  basketCircle.innerHTML = selectedItemCount.toString();
  basketCircleMobile.innerHTML = selectedItemCount.toString();
}
...
function addListenerOnProductCounter() {
  const increasedElementCountButtons =
    document.querySelectorAll(".counter_plus");
  const decreaseElementCountButtons =
    document.querySelectorAll(".counter_minus");
...
function addListenerOnCheckboxes() {
  ...
  const paymentCheckboxComment = document.getElementById(
    "payment_checkbox_comment"
  );
...
function changeQuantityBasedPrice(id) {
  ...
  const finalPriceElementContainer = document.querySelectorAll(
    ".product_cost_container"
  )[productIndex];
  const finalPriceElementMobileContainer = document.querySelectorAll(
    ".product_cost_container_mobile"
  )[productIndex];
  const price = finalPriceElementContainer.querySelector(".cost");
  const priceWithoutDiscount = finalPriceElementContainer.querySelector(
    ".cost_without-discount"
  );
  const mobilePrice = finalPriceElementMobileContainer.querySelector(".cost");
  const mobilePriceWithoutDiscount =
    finalPriceElementMobileContainer.querySelector(".cost_without-discount");
...
function rerenderCard() {
  ...
  const cardImgMobile = document.getElementById(
    "img_card_on_payment_mobile_container"
  );
  const cardImgOnBasket = document.getElementById(
    "img_card_on_payment_basket_container"
  );
  const index = Payment.selectedCardIndex;

  cardSpan.forEach((elem) => {
    elem.textContent =
      Payment.cards[index].card_number + " " + Payment.cards[index].card_day;
  });
